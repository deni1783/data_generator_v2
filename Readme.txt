...
Если в качестве параметра была указана директория с файлами DDL то все вышесказанные действия применяются ко всем
файлом из директории последовательно!

Всвязи с такой спецификой объединения файлов, при  MERGE_ALL_FILE_TO_SINGLE = True необходимо в 2-ва раза больше место
на диске, т.к. происходит дублирование файлов



Запускать нужно из cmd (либо terminal на linux)
(в linux после установки python, необходимо в терминале прописывать вместо python - python3, т.к.
    по умолчанию там уже установлен интерпритатор версии 2.x)
...
Первый параметр (обязательный):
  ...
  Либо путь к папке с несколькими файлами создания таблиц,
      тогда генерация будет запускаться последовательно для каждого файла из папки
...
Парсинг происходит по такому принципу:
  ...
  Все что находится после имени поля и до:
      not null
      null
      либо если нет ни того ни другого, то до конца строки
...
P.S. конструкции "create table" пропускаются,
  так же как и пустые строки, либо строки состоящие только из "(" или ")"
